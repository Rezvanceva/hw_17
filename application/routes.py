...
genres_nc: Namespace = api.namespace('genres')
...
class MovieView(Resource):
    ...
    def delete(self, movie_id):
        deleted_row = db.session.query(models.Movie).filter(models.Movie.id == movie_id).delete()
        if deleted_row != 1:
            return None, 400
        db.session.commit()
        return None, 200
...
class DirectorView(Resource):
    ...
    def delete(self, director_id):
        deleted_row = db.session.query(models.Director).filter(models.Director.id == director_id).delete()
        if deleted_row != 1:
            return None, 400
        db.session.commit()
        return None, 200
...
class GenreView(Resource):
    ...
    def delete(self, genre_id):
        deleted_row = db.session.query(models.Genre).filter(models.Genre.id == genre_id).delete()
        if deleted_row != 1:
            return None, 400
        db.session.commit()
        return None, 200
